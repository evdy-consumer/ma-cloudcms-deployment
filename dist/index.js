...
function getEnvUrl(_ref) {
  var env = _ref.env,
    projectId = _ref.projectId,
    _ref$module = _ref.module,
    id = _ref$module.id,
    timestamp = _ref$module.timestamp;
  var modUrl = "".concat(window.location.origin, "/_modules-").concat(timestamp, "/").concat(id);
  if (projectId) {
    return "".concat(modUrl, "/projects/").concat(projectId, "/").concat(env, ".js");
  }
...
function loadEnvScript() {
  ...
  var onLoad = function onLoad() {
    queuedExecIndexCalls.forEach(function (_ref2) {
...
function loadEnv() {
  var currentBranch = ratchet_web__WEBPACK_IMPORTED_MODULE_0___default.a.observable('branch').get();
  var reload = function reload(newBranch) {
    var currentBranchId = currentBranch && currentBranch._doc;
    var newBranchId = newBranch && newBranch._doc;
    var loadingTheSameBranch = Boolean(currentBranchId) && currentBranchId === newBranchId;
    if (!newBranchId) {
      return;
    }

    // First branch hydration should establish baseline without forcing a reload.
    if (!currentBranchId) {
      currentBranch = newBranch;
      return;
    }
    if (loadingTheSameBranch) {
      return;
    }
    currentBranch = newBranch;
