  ...
  init: function init(editor) {
    ...
    function smartRemoveColorFromPartial(range) {
      ...
      while (textNode = walker.next()) {
        ...
        var parentChain = [];
        var node = textNode.getParent();
        while (node && !node.equals(colorSpan)) {
        ...
        if (beforeFrag) {
          ...
          colorSpan.insertBeforeMe(wrapWithAncestors(span.append(beforeFrag) || span, parentChain));
          console.log('⬅️ Inserted left part with original color and structure');
        }
        if (selectedFrag) {
          var stripped = cloneWithChildrenPreserved(colorSpan, selectedFrag);
          colorSpan.insertBeforeMe(wrapWithAncestors(stripped, parentChain));
          console.log('🆕 Inserted middle with color removed, styles preserved');
        }
        if (afterFrag) {
          var _span = new CKEDITOR.dom.element('span');
          _span.setAttribute('style', colorSpan.getAttribute('style'));
          colorSpan.insertBeforeMe(wrapWithAncestors(_span.append(afterFrag) || _span, parentChain));
          console.log('➡️ Inserted right part with original color and structure');
        }
        textNode.remove();
