...
(function () {
  ...
  CKEDITOR.plugins.add('indentblock', {
    ...
    init: function init(editor) {
      var globalHelpers = CKEDITOR.plugins.indent,
          classes = editor.config.indentClasses; // Register commands.
        ...
        this.jobs = {
          '20': {
            refresh: function refresh(editor, path) {
              var firstBlock = path.block || path.blockLimit; // Switch context from somewhere inside list item to list item,
              ...
              if (firstBlock.is($listItem)) firstBlock = firstBlock.getParent(); //	[-] Context in the path or ENTER_BR
              //
              ...
              if (!this.enterBr && !this.getContext(path)) return TRISTATE_DISABLED;else if (classes) {
                ...
                //		the command.
                if (indentClassLeft.call(this, firstBlock, classes)) return TRISTATE_OFF;else return TRISTATE_DISABLED;
              } else {
                //	[+] Context in the path or ENTER_BR
                //	[-] IndentClasses
                //	[+] Indenting
                ...
                if (this.isIndent) return TRISTATE_OFF; //	[+] Context in the path or ENTER_BR
                //	[-] IndentClasses
                //	[-] Indenting
                //	[-] Block in the path
                //
                ...
                else if (!firstBlock) return TRISTATE_DISABLED; //	[+] Context in the path or ENTER_BR
                  //	[-] IndentClasses
                  //	[-] Indenting
                  //	[+] Block in path.
                  //
            ...
            exec: function exec(editor) {
              var selection = editor.getSelection(),
                  ...
                  nearestListBlock; // If there's some list in the path, then it will be
              // a full-list indent by increasing or decreasing margin property.

              if (nearestListBlock = editor.elementPath().contains($list)) indentElement.call(this, nearestListBlock, classes); // If no list in the path, use iterator to indent all the possible
  ...
  function indentClassLeft(node, classes) {
    var indentClass = node.$.className.match(this.classNameRegex),
        isIndent = this.isIndent; // If node has one of the indentClasses:
    ...
    if (indentClass) return isIndent ? indentClass[1] != classes.slice(-1) : true; // If node has no class which belongs to indentClasses,
...
/******/ })});;
//# sourceMappingURL=bp-hyd-2922-add-coursse-bundle-to-course-landing.js.map